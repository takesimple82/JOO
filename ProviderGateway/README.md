...
This slice delivers only the **additive market-adapter path** authorized for
Prerequisite A / S4. The PF-M1 broker-first first slice (KB Open API,
`broker_fact` production, `adapters/kb_open_api.py`) remains a frozen identity
that this slice does **not** deliver.

Rejected sibling names remain forbidden: `MarketAdapter`, `MarketGateway`,
`QuoteIngress`, `MarketFactStore`.
...
`adapters/kb_open_api.py` is reserved and omitted. S4 does not occupy that
file.
...
| `collect(...)` / `health()` | Provider Interface methods on `MarketApiAdapter` |
| `RESERVED_KB_OPEN_API_PROVIDER_ID` | `"kb_open_api"` |
...
- KB Open API broker-first collect / `broker_fact` production
